			...
			try{
				...
				for(int i = 0; i< methods.length ;i++){
					...
					if(annotation != null){
						...
						zooVO.setName(EgovStringUtil.isNullToString(annotation.name()));
						zooVO.setOrder(EgovStringUtil.isNullToString(annotation.order()).equals("")? 0 : annotation.order());
						zooVO.setGid(EgovStringUtil.isNullToString(annotation.gid()).equals("")? 0 : annotation.gid());
						
						rmAnnotation = methods[i].getAnnotation(RequestMapping.class);
						if("".equals(annotation.listUrl())){
							zooVO.setListUrl(EgovStringUtil.isNullToString(rmAnnotation.value()[0]));
						}
						else{
							zooVO.setListUrl(EgovStringUtil.isNullToString(annotation.listUrl()));
						}
						int oNum = zooVO.getOrder();
						map.put(String.valueOf(oNum), zooVO); //map.put(zooVO.getOrder(),zooVO);
					}
				}
			}
			catch(Exception e){
